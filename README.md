...
Calculate vertical STEP placement from transformed topological vertices, not from every `CARTESIAN_POINT` and not from the absolute lowest STEP vertex alone. STEP files can contain spline/control/reference points far outside the visible model; those must not drive Z placement. After applying the intended Altium model rotations, first split the STEP topology into connected vertex components. If the model has many repeated small components near the projected model perimeter, treat those as solder-contact candidates and choose the lowest significant Z plane from those repeated components. This prevents dense plastic or internal pocket planes from being mistaken for the board-contact plane. If no repeated contact components are found, fall back to grouping all transformed topological vertex Z values into 0.001 mm planes, ignoring sparse outliers, and choosing the lowest significant plane for that model. For footprints in the same connector family, role, suffix, and body height, normalize repeated-contact results by learning the dominant `body_height - model_z` offset from plausible sibling models, then applying that height-line to lower contact-plane clusters. If a model already exposes the highest plausible raw contact plane within its height group, preserve that class, but when it is still far below the family height-line, nudge it a small fraction toward the height-line instead of snapping fully to consensus. If a model's raw contact-derived Z is larger than the body height, keep that raw value because the vendor STEP origin is shifted relative to its siblings. Do not apply family consensus to dense-plane fallback results. The contact plane can be negative or positive depending on the vendor STEP origin. Use `model_z_mm = -contact_plane_z_mm` as the Generic 3D Model Z/Standoff Height value, and keep the existing X/Y placement and model rotations unchanged. In generated diagnostics, `computed_model_z_mm` is the value to pass as the `3D_BODY_SET_PLACEMENT` model-Z argument, `computed_model_z_mm_raw` is the per-model value before family consensus, `computed_model_z_source` shows whether family consensus changed it, `computed_contact_plane_method` shows whether the value came from repeated contact components or the dense-plane fallback, and `computed_body_standoff_height_mm` should remain `0`.
